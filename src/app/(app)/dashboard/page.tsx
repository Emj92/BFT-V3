...
import { Users, Globe, CheckSquare, AlertTriangle, TrendingUp, Clock, Plus, Eye, CheckCircle, BarChart3, Inbox } from "lucide-react"
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        if (selectedWebsite) {
          ...
          // Zusätzlich: Lade lokale Scan-Daten aus localStorage
          const localScans = JSON.parse(localStorage.getItem('website-scans') || '[]')
          console.log('Lokale Scans geladen:', localScans)
          
          // Kombiniere API-Daten und lokale Daten
          ...
          // Verarbeite lokale Scans
          const totalLocalScans = localScans.length
          const totalUniqueWebsites = new Set([
            ...apiWebsites.map((w: any) => w.name),
            ...localScans.map((s: any) => s.website)
          ]).size
          
          if (totalApiScans > 0 || totalLocalScans > 0) {
            const totalWebsites = Math.max(totalUniqueWebsites, 1)
            const totalScans = totalApiScans + totalLocalScans
            
            // Kombiniere Scores von API und lokalen Scans
            const allScores = [
              ...apiScans.map((scan: any) => scan.score || 0),
              ...localScans.map((scan: any) => (scan.score || 0) / 100) // Lokale Scores sind in %, API in 0-1 
            ]
            
            const avgScore = allScores.length > 0 ? allScores.reduce((sum: number, score: number) => sum + score, 0) / allScores.length : 0
            
            // Kombiniere kritische Issues von API und lokalen Scans
            ...
            const localCriticalIssues = localScans.reduce((sum: number, scan: any) => sum + (scan.criticalIssues || 0), 0)
            const criticalIssues = apiCriticalIssues + localCriticalIssues
            ...
            // Kombiniere Aktivitäten von API und lokalen Scans
            const apiActivities = apiScans.slice(0, 2).map((scan: any, index: number) => ({
              id: index + 1,
              type: "scan",
              description: `Website-Scan abgeschlossen (API)`,
              timestamp: new Date(scan.createdAt).toLocaleDateString('de-DE'),
              status: scan.status || "abgeschlossen",
              score: scan.score
            }))
            
            const localActivities = localScans.slice(0, 2).map((scan: any, index: number) => ({
              id: index + 100,
              type: "scan", 
              description: `Barrierefreiheits-Scan für ${scan.website}`,
              timestamp: scan.date,
              status: scan.status || "abgeschlossen",
              score: scan.score / 100 // Konvertiere zu 0-1 Range
            }))
            
            const activities = [...apiActivities, ...localActivities].slice(0, 4)
              ...
              {stats.avgScore > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    ...
                    <Chart score={stats.wcagAACompliance} level="AA" />
                  </div>
